...
test: ## Roda testes unitários
	$(COMPOSE_RUN) -e CGO_ENABLED=1 $(APP_SERVICE) go test -v -count=1 -race ./...

test-integration: ## Roda testes de integração (precisa de Qdrant rodando)
	$(COMPOSE_RUN) -e CGO_ENABLED=1 $(APP_SERVICE) go test -v -count=1 -race -tags=integration ./...

test-coverage: ## Roda testes com cobertura
	$(COMPOSE_RUN) -e CGO_ENABLED=1 $(APP_SERVICE) go test -v -count=1 -race -coverprofile=coverage.out ./...
	$(COMPOSE_RUN) -e CGO_ENABLED=1 $(APP_SERVICE) go tool cover -html=coverage.out -o coverage.html
